...
class ImagioScene(VoiceoverScene):
    def construct(self):
        ...
        import textwrap
        title = Text('Classical vs Quantum Computing', font_size=42, color=WHITE, weight=BOLD)
        ...
        l_hdr  = Text('Classical', font_size=28, color=BLUE, weight=BOLD)
        r_hdr  = Text('Quantum', font_size=28, color='#ff6b9d', weight=BOLD)
        ...
        left_list       = ['Uses bits: 0 or 1', 'Sequential gate operations', 'Deterministic output', 'Scales polynomially', 'Room temperature']
        right_list      = ['Uses qubits: superposition', 'Parallel quantum gates', 'Probabilistic measurement', 'Exponential speedup', 'Near absolute-zero cooling']
        narrations_data = ['Classical computers use bits while quantum computers exploit superposition via qubits.', 'Classical gates run sequentially whereas quantum gates operate in parallel.', 'Classical results are deterministic but quantum measurement is probabilistic.', 'Classical scaling is polynomial while quantum algorithms can offer exponential speedup.', 'Classical machines run at room temperature but quantum hardware needs near absolute zero.']
