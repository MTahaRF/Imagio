...
client = OpenAI(
    api_key="anything", # ð PROXY KEY (can be anything, if master_key not set)
    base_url="http://0.0.0.0:4000" # ð PROXY BASE URL
)
...
|-------|-------------|-------------------------------|
| Gemini 2.0+ | `responseJsonSchema` (JSON Schema) | â Yes |
| Gemini 1.5 | `responseSchema` (OpenAPI) | â No |
